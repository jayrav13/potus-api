...
pages = {
	...
	"/briefing-room/weekly-address": "Your Weekly Address"
}
...
for key, value in pages.iteritems():
	...
	for i in range(0, pagecount):
		...
		try:
			objects = [{
				"document_date": (x.xpath('div[contains(@class, "views-field-created")]')[0].text_content().strip(" ") if len(x.xpath('div[contains(@class, "views-field-created")]')) > 0 else (x.xpath('div')[0].text_content().split(' on ')[1] if len(x.xpath('div')) > 0 else None)),
